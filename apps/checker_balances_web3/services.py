...
class Web3Middleware:
    ...
    def get_native_balance_multicall(self, multicall_eth_contracts: str, address_list: list[str], ) -> list:
        ...
        multicall_balances = multicall_contract.functions.balances(
    ...
    def get_token_balance_multicall(self,
        ...
        decimals_list = [contract.functions.decimals().call() for contract in contracts_list]
        multicall_contract = self.web3.eth.contract(
            address=self.checksum_address(multicall_eth_contracts), abi=MULTICALL_ABI
        )
        multicall_balances = multicall_contract.functions.balances(
